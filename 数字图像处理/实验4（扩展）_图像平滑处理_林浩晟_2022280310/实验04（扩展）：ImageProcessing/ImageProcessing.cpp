...
void MaxFilterProcessing(char* oImage, char* nImage, 
                        int wImage, int hImage)
{
    memcpy(nImage, oImage, wImage * hImage);
    
    for (int y = 1; y < hImage - 1; y++) {
        for (int x = 1; x < wImage - 1; x++) {
            unsigned char maxVal = 0;
            for (int dy = -1; dy <= 1; dy++) {
                for (int dx = -1; dx <= 1; dx++) {
                    unsigned char current = 
                        static_cast<unsigned char>(oImage[(y + dy)*wImage + (x + dx)]);
                    if (current > maxVal) 
                        maxVal = current;
                }
            }
            nImage[y*wImage + x] = static_cast<char>(
                (maxVal > 127) ? maxVal - 256 : maxVal); // 处理溢出
        }
    }
}
