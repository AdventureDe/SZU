...
void ThresholdImage(char *oImage, int wImage, int hImage);
...
#define SHUTTERNUM 10 
#define HISTOGRAM_HEIGHT 150  // 修正拼写错误
#define BACKGROUND_GRAY 192
#define HISTOGRAM_BLACK 0
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{
		case IDM_1:                   //直方图
			ImageHistogramShowProcessing(OrgImage,NImage,IMAGEWIDTH,IMAGEWIDTH);
			ShowImage(NImage, IMAGEWIDTH, IMAGEHEIGHT, 4*XPOS, YPOS);
			break;
		case IDM_2:                   //灰度翻转
			ImageGrayReversionProcessing(OrgImage,IMAGEWIDTH,IMAGEWIDTH);
			ShowImage(OrgImage, IMAGEWIDTH, IMAGEHEIGHT, 4*XPOS, YPOS);
			break;
		case IDM_3:                   //二值化
			ThresholdImage(OrgImage,IMAGEWIDTH, IMAGEWIDTH);
			ShowImage(OrgImage, IMAGEWIDTH, IMAGEHEIGHT, 4*XPOS, YPOS);
			break;
...
{
	//请在下方填入显示RAW格式图像具体代码
	if (!hWinDC) return; // 确保窗口设备上下文有效

	for (int i = 0; i < hImage; i++) {
		for (int j = 0; j < wImage; j++) {
			BYTE pixel = Image[i * wImage + j]; // 取灰度值
...
void OpenImageFileDlg(char* OPDLTitle)
{
	...
	ofn.hwndOwner = NULL;
	ofn.hInstance = NULL;
	ofn.lpstrFilter = TEXT("RAW files (*.raw)\0*.raw\0BMP files (*.bmp)\0*.bmp\0All Files (*.*)\0*.*\0\0"); // 支持 RAW 和 BMP
	...
	if (GetOpenFileName(&ofn)) {
		getcwd(ImgDlgFileDir, MAX_PATH); // 获取当前目录
	}
}
/************************************************************************************************
*																								*
...
void ImageHistogramShowProcessing(char *oImage, char *nImage, int wImage, int hImage)
{
    ...
    int GrayCount[256]; // 灰度级统计数组
    
    // 1. 初始化灰度统计数组
    for (i=0; i<256; i++) GrayCount[i] = 0;
    
    // 2. 统计原始图像灰度分布
    for (i=0; i<wImage*hImage; i++) {
        Gray = (BYTE) oImage[i]; // 提取像素灰度值
        GrayCount[Gray]++; // 对应灰度级计数
    }
    ...
    for (j=0; j<256; j++) {
        ...
        // 从下往上绘制黑色柱状图 (255-i实现Y轴翻转)
        for (i=0; i<GrayCount[j]; i++)
            nImage[(255-i)*256 + j] = 0; // 256为输出图宽度
    }
}
