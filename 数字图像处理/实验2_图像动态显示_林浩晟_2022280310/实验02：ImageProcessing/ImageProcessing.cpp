...
void ShowTopBottomShutter1(char* Image, int wImage, int hImage, int xPos, int yPos);
void ShowLeftRightOpen1(char* Image, int wImage, int hImage, int xPos, int yPos);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDM_TBSHUTTER:				           //由上向下百叶窗
			...
			break;
		case IDM_1:                   //淡出
			ShowGrayToLow1(OrgImage, ImageWidth, ImageHeight, XPOS, YPOS);
			break;
		case IDM_2:                   //淡出
			ShowLeftRightOpen1(OrgImage, ImageWidth, ImageHeight, XPOS, YPOS);
			break;
		case IDM_3:                   //淡出
			ShowTopBottomShutter1(OrgImage, ImageWidth, ImageHeight, XPOS, YPOS);
			break;
...
void ShowLeftRightOpen1(char* Image, int wImage, int hImage, int xPos, int yPos)
{
	...
	for (i = 0; i < wImage / 2; i++) {
		ShowRect.left = wImage / 2 - i;            // 左边界逐渐向左扩展
		ShowRect.right = wImage / 2 - i + 1;        // 显示一列
		ShowImageRect1(Image, wImage, hImage, xPos, yPos, ShowRect);  // 左开门

		ShowRect.left = wImage / 2 + i;            // 右边界逐渐向右扩展
		ShowRect.right = wImage / 2 + i + 1;        // 显示一列
		ShowImageRect1(Image, wImage, hImage, xPos, yPos, ShowRect);  // 右开门
...
void ShowTopBottomShutter1(char* Image, int wImage, int hImage, int xPos, int yPos)
{
	...
	for (i = 0; i < hImage; i++) {
		...
		// 调用 ShowImageRect 函数显示当前行
		ShowImageRect1(Image, wImage, hImage, xPos, yPos, ShowRect);
