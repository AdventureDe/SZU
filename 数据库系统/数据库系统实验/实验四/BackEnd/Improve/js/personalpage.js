...
function updateUserInfo() {
    ...
    if (userId) {
        fetch(`http://localhost:5000/api/users/${userId}`)
            ...
            .then(data => {
                if (data.success) {
                    const username = data.username; // 从后端返回的用户名
...
function addProduct() {
    ...
    // 发送请求到后端
    fetch('http://localhost:5000/api/add_products', {
            method: 'POST',
            headers: {
...
function loadProducts() {
    ...
    fetch(`http://localhost:5000/api/products?user_id=${userId}`) // 通过用户 ID 获取该用户的商品
        .then(response => {
...
function removeProduct(productId) {
    ...
    // 发送 DELETE 请求到后端
    fetch(`http://localhost:5000/api/products/${productId}/remove`, {
