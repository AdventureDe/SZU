...
function loadCartItems() {
    ...
    // 发送请求获取购物车商品
    fetch(`http://localhost:5000/api/cart?user_id=${user_id}`, {
            method: 'GET',
            headers: {
...
function deleteItem(event) {
    ...
    // 删除购物车商品
    fetch(`http://localhost:5000/api/cart/${productId}?user_id=${userId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            },
...
payOrderBtn.addEventListener('click', function () {
    ...
    fetch(`http://localhost:5000/api/order/${orderId}/pay`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
