...
void MeshPainter::bindObjectAndData(TriMesh* mesh, openGLObject& object, const std::string& texture_image, const std::string& vshader, const std::string& fshader) {
    ...
    glGenVertexArraysAPPLE(1, &object.vao);		
...
void MeshPainter::drawMesh(TriMesh* mesh, openGLObject& object, Light* light, Camera* camera, int flag) {
    ...
    // 相机矩阵计算
    camera->updateCamera();  // 更新相机的位置和方向
    camera->viewMatrix = camera->getViewMatrix();  // 获取相机的视图矩阵
    camera->projMatrix = camera->getProjectionMatrix(true);  // 获取相机的投影矩阵
    ...
    // 物体的变换矩阵
    glm::mat4 modelMatrix = mesh->getModelMatrix();  // 获取物体的模型矩阵
    // 传递矩阵到着色器
    glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &modelMatrix[0][0]);  // 将模型矩阵传递给着色器
    glUniformMatrix4fv(object.viewLocation, 1, GL_TRUE, &camera->viewMatrix[0][0]);  // 将视图矩阵传递给着色器
    glUniformMatrix4fv(object.projectionLocation, 1, GL_TRUE, &camera->projMatrix[0][0]);  // 将投影矩阵传递给着色器

    // 根据纹理绘制
    glUniform1i(object.shadowLocation, 2);  // 设置阴影相关的 uniform 值
    glActiveTexture(GL_TEXTURE0);  // 激活纹理单元 0
    glBindTexture(GL_TEXTURE_2D, object.texture);  // 绑定纹理
    // 将纹理数据传递给着色器
    glUniform1i(glGetUniformLocation(object.program, "texture"), 0);  // 将纹理单元 0 传递给着色器

    // 将材质和光源数据传递给着色器
    bindLightAndMaterial(mesh, object, light, camera);

    // 绘制物体
    glDrawArrays(GL_TRIANGLES, 0, mesh->getPoints().size());  // 使用 GL_TRIANGLES 绘制物体

    // 计算阴影投影矩阵
    glm::vec3 light_position = light->getTranslation();  // 获取光源的位置
    float lx = light_position[0];  // 光源位置的 x 坐标
    float ly = light_position[1];  // 光源位置的 y 坐标
    float lz = light_position[2];  // 光源位置的 z 坐标
    glm::mat4 shadowProjMatrix(
        -ly, 0.0, 0.0, 0.0,  // 阴影投影矩阵的前 4 个元素
        lx, 0.0, lz, 1.0,     // 阴影投影矩阵的后 4 个元素
        0.0, 0.0, -ly, 0.0,   // 阴影投影矩阵的第三行
        0.0, 0.0, 0.0, -ly);  // 阴影投影矩阵的第四行

    // 计算阴影的模型变换矩阵
    modelMatrix = shadowProjMatrix * modelMatrix;  // 计算阴影的变换矩阵
    if (flag != -1) {  // 如果 flag 不为 -1，表示需要绘制阴影
        glUniform1i(object.shadowLocation, 3);  // 设置阴影的 uniform 值
        // 传递阴影的变换矩阵给着色器
